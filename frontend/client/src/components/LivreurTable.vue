    ...
    <v-card-text>
      <v-container>
        <v-form ref="editForm">
          ...
          <v-switch v-model="editedItem.disponibility" label="disponibility"></v-switch>
        ...
        <v-date-picker :max="new Date().toISOString().substr(0, 10)"
      min="1950-01-01"
      @change="save">
        </v-date-picker>
        </v-form>
...
<v-dialog v-model="consultDialog" max-width="500px">
  <v-card>
    ...
    <v-card-actions>
      <v-spacer></v-spacer>
      <v-btn color="blue darken-1" text @click="consultDialog = false">Close</v-btn>
    </v-card-actions>
  </v-card>
</v-dialog>
  ...
  data() {
    return {
      headers: [
        ...
        { text: 'disponibilité', value: 'disponibility' },
